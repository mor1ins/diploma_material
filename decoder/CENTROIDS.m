...
% It calls a number of functions to do incline-correction reconstruct the virtual % raster lines and decoding.
...
OI = imread(FullFileName); OI = flipud(OI);
figure
%imshow(OI);
[numofBlobs, centroids] = GetCentroids(OI);
%}
% Since centroids is a N×2 array, it's not easy and instinct to utilize % its data, we set two new N×N arrays x and y to store x-coordinates and % y-coordinates of the centroids separately.
N = sqrt(numofBlobs);
xt = zeros(N); yt = zeros(N); for n=1:N
for m=1:N
xt(m,n) = centroids(m+(n-1)*N,1); yt(m,n) = centroids(m+(n-1)*N,2);
end end
[temp,index]=sort(yt,'descend'); % create y matrix
74
y = temp;
% create x matrix
x = zeros(N); for m=1:N
for n=1:N x(n,m)=xt(index(n,m),m);
end end
% obtain the offset indicators recX and recY % obtain the spacing in x and y directions
[recX,recY,xSpace,ySpace] = dotDirec(x,y); [averx,avery,~,~] = Gridestab(recX,recY,x,y);
for i = 1:N for j = 1:N
...
xcode = zeros(N); ycode = zeros(N); for j =1:N
for i = 1:N
switch MarkVal(j,i)
75
...
[objectX,objectY] = getcent objectR = 0;
objectC = 0;
for i = 1:N-1
...
BitSeq = enCod(0,6);
PaSeq_1SDS = PaSeq_2SDS = PaSeq_3SDS = PaSeq_4SDS = % decoding.. % obtain the [PVy,PDSy] = [PVx,PDSx] =
enCod(1,5); enCod(2,5); enCod(3,5); enCod(4,5);
PDS
deCody(ycode, BitSeq, 6); deCodx(xcode, BitSeq, 6);
% convert PDS into SDS in x direction
[a1x,a2x,a3x,a4x] = PDScoeff(PDSx);
% obtain the place numbers in SDS in x direction
[p1x,~] = deCodx(a1x,PaSeq_1SDS ,5); [p2x,~] = deCodx(a2x,PaSeq_2SDS ,5); [p3x,~] = deCodx(a3x,PaSeq_3SDS ,5); [p4x,~] = deCodx(a4x,PaSeq_4SDS ,5);
% convert PDS into SDS in y direction
[a1y,a2y,a3y,a4y] = PDScoeff(PDSy);
76

% obtain the place numbers in SDS in y direction
[p1y,~] = deCody(a1y,PaSeq_1SDS ,5); [p2y,~] = deCody(a2y,PaSeq_2SDS ,5); [p3y,~] = deCody(a3y,PaSeq_3SDS ,5); [p4y,~] = deCody(a4y,PaSeq_4SDS ,5);
% obtain the place numbers in PDS if needed %Px = Place_PDS(xcode,1);
...
Ax = [32,57,12,96]; Ay = [224,19,4,155];
% obtain the coordinates in columns and rows of the ROI.
X = DetCor(Ax,Bx); Y = DetCor(Ay,By);
% obtain the coordinates in columns and rows of the object.
objectC = X + objectC -1; objectR = Y + objectR -1; Coordinate = [objectC,objectR]
