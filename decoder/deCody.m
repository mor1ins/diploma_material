% Title: deCody.m
% [PosVal,DS] = deCody(code, BitSeq, n)decodes the code in the y direction % n— has the value 5, 6 or 8. n=5 indicates encoding SDS, n=6 or 8
% indicate encoding PDS using 6×6 array or 8×8 array.
function [PosVal,DS] = deCody(code, BitSeq, n) [r_code,c_code] = size(code);
switch n
case 8
c_codeT = floor(c_code/n); % number of vertical bit sequences of x-code. r_codeT = r_code;
...
switch n
   case 8
       for
i = 1: c_codeT
code8 = char(int2str(code(:,1+(i-1)*n:i*n))); [~,c8] = size(code8);
...
case 5 for
i = 1: c_codeT
code6 = char(int2str(code(:,1+(i-1)*n:i*n))); [~,c6] = size(code6);
bla(:,1+(i-1)*c6:i*c6) = code6;
i = 1: r_codeT
code5 = char(int2str(code(1+(i-1)*n:i*n,:)')); [r5,c5] = size(code5);
for j = 1:r5
